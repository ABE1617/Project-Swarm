...
export default function CredentialsModal({ onClose }: { onClose: () => void }) {
  ...
  async function connect(credentialId: number) {
    const { url } = await api.get<{ url: string }>(
      `/api/credentials/${credentialId}/oauth/start`,
    )
        ...
        {creating ? (
          <form className="credential-form" onSubmit={handleCreate}>
            ...
            <div className="credential-hint">
              ...
              , create an OAuth client (type: Web application) and add this redirect URI:
              <code>{redirectUri}</code>
              Then enable the APIs for the services you selected.
            </div>
            ...
            <div className="credential-form-actions">
              ...
              <button type="submit" className="btn btn-primary" disabled={busy || services.length === 0}>
